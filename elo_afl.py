...
class Elo:
	...
	def initial_ranking(self, row, home_team, away_team, mean_reversion):
		''' Calculates the current rankings of the teams including update
		...
		Returns:
			(home_ranking, away_ranking) -> Current home and away team rankings
		'''

		if row['Round']  == 1:
			home_ranking = 1500 + (self.current_ratings[home_team] - 1500) * mean_reversion
			self.current_ratings[home_team] = home_ranking
			away_ranking = 1500 + (self.current_ratings[away_team] - 1500) * mean_reversion
			self.current_ratings[away_team] = away_ranking
		else:
	...
	def run_model(self, k, home_field,mean_reversion,margin_smoothing):
		...
		for row_index, row in df.iterrows():
			...
			home_ranking, away_ranking = self.initial_ranking(row, home_team, away_team, mean_reversion)
			...
			if home_score > away_score:
				...
				actual_result = 0
			new_rank = self.elo_calculation(k,point_differential,old_rank,opponent_rank, winners_rank,losers_rank,actual_result, home_field_advantage,margin_smoothing)
